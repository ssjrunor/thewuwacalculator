/*
  Author: Runor Ewhro
  Description: Owns left behavior and state transitions for the main module.
*/
...
const LazyRotPane = lazy(async () => ({
  default: (await import('@/modules/simulation/features/rotation/sequence-editor/Pane.tsx')).RotationSequenceEditor,
}))
...
const LazyCalcChsP = lazy(async () => ({
  default: (await import('@/modules/simulation/features/echoes/Pane.tsx')).Echoes,
}))

const LazyCalcEnem = lazy(async () => ({
  default: (await import('@/modules/simulation/features/enemies/Pane.tsx')).EnemyPane,
}))

const LazyCalcSugg = lazy(async () => ({
  default: (await import('@/modules/simulation/features/suggestions/Pane.tsx')).Suggestions,
}))

const LazyCalcTmsP = lazy(async () => ({
  default: (await import('@/modules/simulation/features/teams/Pane.tsx')).Teams,
}))

const LazyCalcWpnP = lazy(async () => ({
  default: (await import('@/modules/simulation/features/weapons/Pane.tsx')).Weapon,
}))
