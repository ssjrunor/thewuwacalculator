/*
  Author: Runor Ewhro
  Description: Splits what the stats tree holds and the stat sheet never prints
               into the two different things it actually is.

               The sheet reads eighteen rows, and for a scope it only ever reads
               DMG Bonus. So a bucket like "Heavy Attack, DEF Ignore" has no row
               anywhere to land on, and RES Shred has no unconditional form at
               all -- it exists only where a scope reaches it.

               What comes out is two halves. Modifiers that qualify a stat the
               sheet already states -- Crit Rate and Crit DMG -- come back as
               addends against that row, to be disclosed under it. Everything
               else comes back as a row of its own, so the same disclosure can
               reach it.
*/
