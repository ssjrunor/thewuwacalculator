...
import {featureMeta} from "@/modules/simulation/features/rotation/shared/featureCatalog.ts";
...
function getCtnNegFfc(
  ...
  const hasTtchStckC = appliedChanges.some((change) => getCmbtKeyFo(change.path) === key)
  if (
    !hasTtchStckC &&
...
function mkCtnEnt(
  ...
  const rules = options.rules ?? node.changes ?? []
  const negFfctStck = getCtnNegFfc(node, meta, combatState, rules)

  return {
    key: `${node.id}:${node.featureId}`,
    label: getFeatCtnLb(node, meta),
    multiplier: getFeatMltp(node) * (options.multiplierScale ?? 1),
    resonatorId: partId,
    ...
    rules: rules.map((change): RotSqncRule => ({ type: 'change', change })),
  }
}
...
function cllcStpRls(nodes: RotationNode[]): RotSqncRule[] {
  ...
  for (const node of nodes) {
    ...
    rules.push(...cllcStpRls(getRotNodeItems(node)))
  }
  ...
  const visit = (nodes: RotationNode[], depth = 1, phase: 'setup' | 'body' = 'body') => {
    ...
    for (const node of nodes) {
      if (node.type === 'feature') {
        const attachedChanges = attachedConditionChanges(node, { enabledOnly: true })
        // Attachment writes are an overlay for this one parent/child group.
        // Keep their combat snapshot local so a later root action cannot read it.
        const scopedCombat = attachedChanges.length > 0 ? { ...combatState } : combatState
        if (node.enabled ?? true) {
          applyCombat(scopedCombat, attachedChanges)
        }

        const action = mkCtnEnt(node, input.resonatorId, scopedCombat, {
          rules: attachedChanges,
        })
        actions.push(action)
        ...
        for (const child of node.attached?.features ?? []) {
          ...
          const childAction = mkCtnEnt(child, input.resonatorId, scopedCombat, {
            multiplierScale: action.multiplier,
            rules: [],
          })
      ...
      if (node.type === 'repeat' || node.type === 'uptime') {
        ...
        visit(getRotNodeSetup(node), depth + 1, 'setup')
        visit(getRotNodeItems(node), depth + 1, 'body')
