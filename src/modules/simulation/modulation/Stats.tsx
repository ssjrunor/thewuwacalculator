/*
  Author: Runor Ewhro
  Description: Modulation's stats view. It is the damage view's
               shape, because the two answer the same kind of question: a list
               of rows on the left, and a card beside it that resolves whichever
               row is picked.

               The sheet uses the damage view's devices rather than standing up
               a parallel version of them. Its two-column reader, sticky card,
               group wells, disclosures, row rails, selection beads and moving
               leader all use the same markup and classes. Only the wider value
               grid and stat-specific text remain its own. The six attribute rows
               keep their colour; every other stat uses neutral ink.

               Three readings sit at three depths. The row carries what the two
               ideal builds hold, the label carries a tooltip for what the stat
               earns, and the card resolves the picked stat as literal arithmetic:
               resonator and weapon base, pooled percentage multiplier, every
               flat/additive source, and the final value.
*/
...
import { aggregateSubstats } from '@/data/scoring/substatMath.ts'
...
/*
  a rounding difference is not a spread. the report rounds before it prints, so
  HP can read 14,860 against 14,859 and mean "the same", which a bare inequality
  would show as a difference the reader then has to dismiss by eye.
*/
const SAME_EPS = 0.005

/* the other modifiers belong to no one stat, so their well wears lucide's
   asterisk rather than any one stat's art. the view box is cropped to the
   glyph so it fills the well like the stat icons do. */
...
/*
  the row already carries the label the icon map is keyed by, so the mark reads
  that rather than deriving one from the key: the overview rows key flat stats
  as `atk`, while the label map keys them `atkFlat`, and a derived lookup misses.
  a mark with no icon renders nothing, or its own ground would paint a solid
  square where the glyph should be.
*/
...
/*
  A scoped addend opens under the row it qualifies, the same way a hit's
  sub-hits open under it: the bead still picks the row for the card, the face
  opens the drawer, and the caret says there is one. A badge could only say how
  many; the drawer says which and how much.
*/
...
function StatRow({
  ...
  return (
    ...
    >
      ...
      {/* the face is the row: everything but the bead is one target, the way a
          hit's face is, so a stat is picked by pressing the stat and not by
          finding its bead */}
      <button
...
/*
  What the two target columns were built on. A sonata is a plan, not a set: a
  frame can activate 5, or 3+2, or 2+2+1, so every entry states its own glyph
  and its own count. The count rides the glyph as a badge because a plan has to
  sit beside an Echo name in a 20rem column.
*/
...
export function ModulationStats({
  ...
  /* the frame behind each column, one row each. The two anchors usually build
     the same frame; the column has the room to say so plainly. */
  ...
  /* The damage view's leader belongs to the shared reader layout: keep it in
     the channel and follow the selected stat as groups open and close. */
