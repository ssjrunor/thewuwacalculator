/*
  Author: Runor Ewhro
  Description: Modulation's damage view: what every hit in the
               subject's kit does, and nothing about rotations.

               The list is the Simulation engine's grouping -- one section per
               skill tab, the same Normal / CRIT / AVG order the results pane
               reads in -- with two things the pane could not afford in its
               half of a split. A row's ground carries its hit's damage
               attribute rather than a magnitude, so a mixed kit reads as
               bands. And the bead at the left end says which hit the formula
               beside the list is reading, so the formula is a place on the
               surface instead of a drawer that pushes every row below it down.

               The drawer that remains holds sub-hits and nothing else, which
               is why the global "show sub-hits" switch has no counterpart
               here: a row that has sub-hits says so on its face.
*/
...
import { selEnemyProf } from '@/domain/state/selectors.ts'
...
import { skillDisplayColor, supportSkillStyle } from '@/modules/simulation/features/rotation/shared/skillDisplay.ts'
...
/*
  the enemy line the switch wears, so the view says what it is measuring
