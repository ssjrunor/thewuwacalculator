...
export interface StateNode {
  ...
  ownScpLbl: string
  /**
   * where the node actually came from. `ownerScope` folds echo and echo set
   * together for grouping, so a surface that wants to show the true source,
   * such as a set's own sonata icon, has to read this instead.
   */
  source: DataSrcRef
  stateLabels: string[]
  effectLabels: string[]
  statTerms?: StatSourceTerm[]
}
...
function fmtOwnLbl(owner: SrcOwnDef): string {
  const ownerName = getSourceOwnerName(owner.source, owner.label)

  if (owner.source.type === 'echo') {
    return `Main Echo: ${ownerName}`
  }
...
/* Overview stats are broader than one skill: an element row consumes its own
   attribute bucket plus `all`, while a damage-type row consumes its own bucket
   plus `all`. Keep this translation beside the skill translation so every
   source surface filters the same effect definitions. */
function opTargetsStat(
    operation: EffectOperation,
  ...
  if (operation.type === 'add_attribute_mod') {
    ...
    return operation.mod === 'dmgBonus' && attributes.some((attribute) => attribute === 'all' || attribute === target.key)
  }

  if (operation.type === 'add_skilltype_mod') {
    const skillTypes = Array.isArray(operation.skillType) ? operation.skillType : [operation.skillType]
    return operation.mod === 'dmgBonus' && skillTypes.some((skillType) => skillType === 'all' || skillType === target.key)
  }

  return false
}

function effectTargetsStat(effect: EffectDef, target: StatStateSummaryTarget): boolean {
  return effect.operations.some((operation) => opTargetsStat(operation, target))
}

function statTermsFor(
    effect: EffectDef,
  ...
  return effect.operations.flatMap<StatSourceTerm>((operation) => {
    if (!opTargetsStat(operation, target)) return []

    if (operation.type === 'add_base_stat') {
      return [{
        kind: operation.field === 'percent' ? 'percent' : 'add',
        key: `${operation.stat}${operation.field === 'percent' ? 'Percent' : 'Flat'}`,
        value: evalForm(operation.value, scope),
      } satisfies StatSourceTerm]
    }

    if (operation.type === 'set_final_stat') {
      return [{ kind: 'set', key: operation.stat, value: evalForm(operation.value, scope) } satisfies StatSourceTerm]
    }

    if (operation.type === 'add_top_stat') {
      return [{ kind: 'add', key: operation.stat, value: evalForm(operation.value, scope) } satisfies StatSourceTerm]
    }
...
function fmtOpLbls(
  ...
  const operations = target
      ...
      : statTarget
          ? effect.operations.filter((operation) => opTargetsStat(operation, statTarget))
          : effect.operations
  ...
  return operations.flatMap((operation) => {
    ...
    if (operation.type === 'set_final_stat') {
      const value = formatValue(evalForm(operation.value, scope))
      return withHghl(fmtBaseStatL(operation.stat), 'Set to', value)
    }
...
function buildContext(
  ...
  // when a summary is rendered outside the live Simulation graph path.
  ...
  const groups = sourceIds.flatMap((sourceId) => {
    ...
    const nodes = owners.flatMap((owner) => {
      ...
      const effects = listFfctForO(owner.ownerKey)
          ...
          .filter((effect) => !options.statTarget || effectTargetsStat(effect, options.statTarget))
          .filter((effect) => evalCond(effect.condition, makeEvalScope(ownerContext)))

      const effectLabels = nqStrn(effects.flatMap((effect) => fmtOpLbls(
          effect,
          ownerContext,
          options.skillTarget,
          options.statTarget,
      )))
      const statTerms = options.statTarget
          ? effects.flatMap((effect) => statTermsFor(effect, ownerContext, options.statTarget!))
          : undefined
      ...
      if (owner.source.type === 'echoSet' && effects.length > 0) {
        const ffctByPc = new Map<string, { labels: string[]; terms: StatSourceTerm[] }>()

        for (const effect of effects) {
          const labels = nqStrn(fmtOpLbls(
              effect,
              ownerContext,
              options.skillTarget,
              options.statTarget,
          ...
          const existing = ffctByPc.get(pieceLabel) ?? { labels: [], terms: [] }
          ffctByPc.set(pieceLabel, {
            labels: nqStrn([...existing.labels, ...labels]),
            terms: options.statTarget
                ? [...existing.terms, ...statTermsFor(effect, ownerContext, options.statTarget)]
                : [],
          })
        }

        const effectNodes = Array.from(ffctByPc.entries()).map(([pieceLabel, entry]) => ({
          ...
          source: owner.source,
          stateLabels: [],
          effectLabels: entry.labels,
          statTerms: options.statTarget ? entry.terms : undefined,
        } satisfies StateNode))
