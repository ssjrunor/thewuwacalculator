...
function normEvaluationTune(value: number | null | undefined): number {
  if (!Number.isFinite(value)) {
    return EVALUATION_ENEMY.status?.tuneStrain ?? 0
  }

  return Math.max(0, Math.floor(Number(value)))
}

export function makeEvaluationEnemy(tuneStrain: number | null | undefined): EnemyProfile {
  return {
    ...EVALUATION_ENEMY,
    status: {
      ...(EVALUATION_ENEMY.status ?? { tuneStrain: 0 }),
      tuneStrain: normEvaluationTune(tuneStrain),
    },
  }
}
...
export function applyEvaluationAsm(runtime: ResRuntime): ResRuntime {
