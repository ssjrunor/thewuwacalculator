...
import { MAX_RES_LVL } from '@/engine/runtime/defaults'
...
import { listStatesFor, getOwnForKey } from '@/data/catalog/gameDataService'
...
import { getWeapon, withDefWpnMg, weaponStatsAt } from '@/modules/simulation/features/weapons/lib/weapon.ts'
...
interface OptCardProps {
  ...
  setPrefs?: TeammateSetPreference[] | undefined
...
function fltrSttsForC(
  ...
  return fltrSrcSttsW(
    ...
    (state) => isStateVisible(runtime, runtime, state, actRt),
  )
}
  ...
  if (!mainEchoDef) {
    ...
    return (
      <div className="opt-team__echo-block">
        <button
          type="button" className="opt-team__set-card opt-team__set-card--empty opt-team__set-card--button"
    ...
    <div className="opt-team__echo-block">
      <div className={`opt-team__set-card${hasNvldMainE ? ' opt-team__set-card--invalid' : ''}`}>
        <div className="opt-team__set-head">
          ...
          {onClearMainEcho ? (
            <button
              type="button" className="opt-team__remove-btn"
...
function mkSetSelForP(
  preferences: TeammateSetPreference[],
): OptSetChoice {
  return {
    1: preferences
...
function OptEchoSetCard({
  ...
  const activeSets = useMemo(() => {
    ...
    return listActiveSets(runtime.build.echoes)
              ...
              <div className="opt-team__set-header-right">
                {configurable ? (
                  <div className="opt-team__set-piece-pills">
                    {getTeammateSetCounts(setId).map((option) => (
                      <button
                        key={option}
                        type="button"
              ...
              <div className="opt-team__set-passives">
                {passiveParts.map((part) => (
                  <div key={part.key} className="opt-team__set-passive">
                    ...
                    <RichDscr description={part.description ?? part.label} className="opt-team__set-passive-desc" />
                  </div>
                ))}
              </div>
            ) : null}
      ...
      >
        <div className="co-runtime-state__step-header">
          ...
          <div className="co-runtime-state__step-count">
            <input
              type="number" className="co-runtime-state__step-count-input"
              value={stepValue}
              min={min}
              max={max}
              disabled={!isEnabled}
  ...
  return (
    <article className={`opt-team__card${editableLevel ? ' opt-team__card--active' : ''}`}>
      <div className="opt-team__portrait-row">
        {onPortraitClick ? (
          <button
            type="button" className="opt-team__thumb-ring opt-team__thumb-ring--button"
          ...
          <div className="opt-team__inline-fields">
            ...
            <div className="co-topbar-field">
              ...
              <LiquidSelect
                ...
                options={sqncPtns}
                onChange={(value) => {
                  onRtPdt((prev) => setResRtSequence(prev, getResDtlsBy()[prev.id], value))
                }}
      ...
      <div className="opt-team__weapon">
        ...
        <div className="co-weapon-card">
          <button
            type="button" className="co-weapon-card__icon-wrap co-weapon-card__icon-wrap--button"
            onClick={onWpnClck}
        ...
        {([0, 1] as const).map((slotIndex) => {
          ...
          if (!memberId || !memberRuntime) {
            return (
              <button
                key={slotIndex}
                type="button" className="opt-team__card opt-team__card--empty opt-team__card--empty-button"
