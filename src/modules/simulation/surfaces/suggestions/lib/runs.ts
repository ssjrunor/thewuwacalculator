/*
  Author: Runor Ewhro
  Description: Coordinates suggestion worker jobs, cache identities, cancellation, and result materialization.
*/
...
export async function runCchdSuggJ<TPrepared, TResult>({
  ...
  try {
    ...
    console.error(`[SuggestionsPane] ${logLabel} suggestions failed`, error)
  } finally {
    if (isCrrnSqnc(sequence)) {
      setRunning(false)
    }
