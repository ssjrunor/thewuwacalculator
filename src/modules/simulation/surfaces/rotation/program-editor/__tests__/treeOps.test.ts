...
describe('Off-Tune landing authoring', () => {
  it('moves the one authored landing within a Tune Break segment', () => {
    const sections = section([
      { ...step('break'), kindLabel: 'tuneBreak' },
      { ...step('old'), offTuneResume: true },
      step('later'),
    ])

    const moved = setOffTuneResume(sections, 'later', true)

    expect(findNode(moved, 'old')).toMatchObject({ offTuneResume: undefined })
    expect(findNode(moved, 'later')).toMatchObject({ offTuneResume: true })
  })
})
