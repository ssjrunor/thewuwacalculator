...
import type { OffTuneTrace } from '@/domain/entities/stats.ts'
import type {
  EditorBlock,
  EditorSection,
  EditorStep,
...
describe('fmtStat', () => {
  it('opens with thirteen columns', () => {
  ...
  it('uses the same configured precision as damage cells for absolute stats', () => {
    ...
    expect(fmtStat('6.096/38.4', 'offTune', 2, 'percent')).toBe('6.10/38.40')
    expect(fmtStat('6.096/38.4', 'offTune', 4, 'factor')).toBe('6.0960/38.4000')
...
describe('Off-Tune row state', () => {
  it('exposes a sealed cooldown entry to the register styling', () => {
    expect(offTuneState({ reset: false, sealed: true } as OffTuneTrace)).toBe('sealed')
  })
