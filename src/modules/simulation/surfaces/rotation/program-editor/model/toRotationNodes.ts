/*
  Author: Runor Ewhro
  Description: Converts editor projections back into canonical authored rotation nodes.
*/
...
import { markEditorSection } from '@/modules/simulation/surfaces/rotation/program-editor/model/sections.ts'
import {
  ROT_LOOP_COLORS,
...
function serializeStep(
  ...
  /* the mark is projected from the node, so writing it back is the whole of it */
  if (node.offTuneResume) nextNode.offTuneResume = true
  else delete nextNode.offTuneResume
