...
/** every step in the order the register reads them */
function stepsInOrder(nodes: readonly EditorNode[], out: EditorStep[] = []): EditorStep[] {
  for (const node of nodes) {
    if (node.type === 'step') out.push(node)
    else if (isEditorBlock(node)) stepsInOrder(node.children, out)
  }
  return out
}

/**
 * Whether this step can carry the resume mark, and whether it does.
 *
 * Only the features standing after a Tune Break can: before the first break
 * of a run there is nothing to start counting again from.
 */
export function offTuneResumeScope(
  sections: EditorSection[],
  id: string,
): { marked: boolean } | null {
  const steps = stepsInOrder(sections.flatMap((section) => section.children))
  const at = steps.findIndex((step) => step.id === id)
  if (at < 0 || steps[at]?.kindLabel === 'tuneBreak') {
    return null
  }
  const broken = steps.slice(0, at).some((step) => step.kindLabel === 'tuneBreak')
  return broken ? { marked: Boolean(steps[at]?.offTuneResume) } : null
}

/*
  Where Off-Tune starts counting again after a Tune Break.

  Only one feature can hold that in a break's aftermath, so marking one clears
  the mark from every other step standing between the same two breaks. A break
  left without a mark falls back to holding its default count.
*/
export function setOffTuneResume(
  sections: EditorSection[],
  id: string,
  on: boolean,
): EditorSection[] {
  if (!on) {
    return updateStep(sections, id, (step) => ({ ...step, offTuneResume: undefined }))
  }
  ...
  const breaks = (step: EditorStep) => step.kindLabel === 'tuneBreak'

  let from = 0
  for (let index = at - 1; index >= 0; index -= 1) {
    if (breaks(steps[index]!)) {
      from = index + 1
      break
    }
  }
  let until = steps.length
  for (let index = at + 1; index < steps.length; index += 1) {
    if (breaks(steps[index]!)) {
      until = index
      break
    }
  ...
  for (let index = from; index < until; index += 1) {
    ...
    if (step.id === id || !step.offTuneResume) continue
    next = updateStep(next, step.id, (entry) => ({ ...entry, offTuneResume: undefined }))
  }
  return updateStep(next, id, (step) => ({ ...step, offTuneResume: true }))
}
