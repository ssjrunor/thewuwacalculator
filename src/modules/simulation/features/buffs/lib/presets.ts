  ...
  return Object.values(registry.sourcesByKey)
      ...
      .flatMap((sourcePkg) => {
        ...
        return (sourcePkg.effects ?? []).map((effect): BuffPresetEntry => {
          ...
          const effectLabel = source.type === 'weapon' || source.type === 'echoSet'
            ? effectName
            : cleanPresetEffectLabel(effect.label, name)
...
function makeScope(
  ...
  // controls and weapon rank without mutating the live scenario runtime.
