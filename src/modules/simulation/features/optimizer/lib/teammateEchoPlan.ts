/*
  Author: Runor Ewhro
  Description: Materializes five-slot teammate Echo plans from selected sets
...
function normalizeTeammateSetCount(setId: number, count: number): number {
  const options = getTeammateSetCounts(setId)
  if (options.length === 0) {
    return 0
  }
...
function trimTeammateSetPreferences(
    preferences: TeammateSetPreference[],
): TeammateSetPreference[] {
  let next = preferences
      .filter((entry, index, array) => (
        entry.count > 0
        && getTeammateSetCounts(entry.setId).length > 0
        && array.findIndex((candidate) => candidate.setId === entry.setId) === index
      ))
      .map((entry) => ({
        setId: entry.setId,
        count: normalizeTeammateSetCount(entry.setId, entry.count),
      }))
...
function derSetPrefsF(
  ...
  const preferences = Object.entries(counts)
    .flatMap(([rawSetId, count]): TeammateSetPreference[] => {
  ...
  return trimTeammateSetPreferences(preferences)
}
...
function normPlan(plan: EchoPlan): EchoPlan {
  return {
    ...
    setPrefs: trimTeammateSetPreferences(plan.setPrefs),
  }
}
...
export function setSetCount(
  ...
  return normPlan({
    ...
    setPrefs: [
      {
        setId,
        count: normalizeTeammateSetCount(setId, count),
      },
      ...plan.setPrefs.filter((entry) => entry.setId !== setId),
    ],
  })
