/*
  Author: Runor Ewhro
  Description: The read-only flattened rotation. Every hit the run produced, in
               order, on the same register the authoring list draws: one set of
               tracks, one heading, one set of bands. What is not a hit is drawn
               as what it did to the hits, so a state change is named on the
               hit it conditions and a container is the band its rows sit in.
*/
...
import type { RotationNodeRevealRequest } from '@/modules/simulation/features/rotation/program-editor/interaction/nodeNavigation.ts'
import {
  buildRegister,
  factorAt,
  formatDamage,
...
import {
  makeRoster,
  type TeamLookup,
} from '@/modules/simulation/features/rotation/program-editor/components/NodeList.tsx'
...
export function FlatList({
  ...
  */
  ...
  /*
    the leading glyph and the node column say the same thing. when the reader
    has the column up the glyph stands down rather than repeating it.
  */
