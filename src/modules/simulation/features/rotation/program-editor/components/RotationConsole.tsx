/*
  Author: Runor Ewhro
  Description: Owns rotation console behavior and state transitions for the components module.
*/
...
import { RCN_BAND_LANES } from '@/modules/simulation/features/rotation/program-editor/presentation/consoleModel.ts'
import type {
  RcnBand,
  RcnGlyph,
  RcnModel,
  RcnNode,
  RcnStateTrack,
} from '@/modules/simulation/features/rotation/program-editor/presentation/consoleModel.ts'
import type { FlatRowTarget } from '@/modules/simulation/features/rotation/program-editor/presentation/flatRows.ts'
...
export function RotationConsole({ model, members, decimals, selectedId, onSelect, caption, emptyNote }: Props) {
  ...
  useLayoutEffect(() => {
    ...
    /* measured on the spot rather than waiting on the observer, so the first
       paint after the drawer opens is already the right width */
    const measure = () => {
  ...
  */
  ...
  /*
    the same authored node stands in every pass it ran in. the playhead belongs
    on the one the register is reading, and falls back to the first occurrence
    when the register is parked on a pass this trace never took.
  */
