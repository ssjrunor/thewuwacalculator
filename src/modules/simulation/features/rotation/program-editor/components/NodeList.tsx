...
import {
  ...
  bandCells,
  useScrollbarInset,
} from '@/modules/simulation/features/rotation/program-editor/components/RegisterCells.tsx'
...
import { carryFoldId } from '@/modules/simulation/features/rotation/program-editor/interaction/selection.ts'
import {
  readStickyScopes,
  sameStack,
  scrollToScope,
  type StickyState,
} from '@/modules/simulation/features/rotation/program-editor/interaction/enclosingScopes.ts'
import {
  atRun,
  blockRatioAt,
  blockRunsAt,
...
import { DEFAULT_ROT_BLOCK_COLOR } from '@/modules/simulation/features/rotation/shared/containerMeta.ts'
...
function NoteAside({
  ...
  /* two frames, not one: the card mounts inside the pointer event, so the
     opened class has to wait for the closed state to actually paint or there
     is nothing for the transition to run from */
  ...
  /* the bubble moves under the card whenever anything it sits in scrolls, so
     the placement is measured again rather than remembered */
  useLayoutEffect(() => {
    if (!visible) return
    const place = () => {
...
export function useNodeDrag({
  ...
  */
  ...
  /*
    the closing edge is dragged, not the block: what moves is where the bracket
    stops, so nothing is picked up and nothing is set down beside anything.
  */
  ...
  const props = (id: string, node: EditorNode, opts: DragOpts = {}) => ({
    ...
    onDragStart: (event: React.DragEvent) => {
      ...
      /*
        In selection mode the row grabbed is only the handle: what is in hand
        is the whole picked set, named by the row the reader sees first.
      */
  ...
  /*
    a section's body takes what lands in it but not on any node, which is the
    only way into a section holding nothing. it sits under the nodes, so a drop
    on a row still lands beside that row rather than at the end.
  */
    ...
    return (
      ...
      >
        ...
        */}
        ...
        {/*
          the features this step carries. they are features, so they take the
          row a feature takes, every column included. they sit in from the
          parent and nothing else marks them: the list indents nothing else, so
          the indent has no other meaning to be read as.

          they are drawn inside the parent's own row rather than as rows of
          their own, which keeps one interactive element per group: selecting
          anywhere in it opens the parent, which is where a carried feature's
          multiplier is authored.
        */}
        {carried.length > 0 ? (
          <Expandable
    ...
    >
      ...
      */}
      ...
      <div className="rte-rows rte-scroll" ref={scrollerRef}>
        {/*
          the enclosing scopes, restated. it sits in the scroller as a sticky
          box of zero height so it overlays the rows instead of shifting them.
        */}
