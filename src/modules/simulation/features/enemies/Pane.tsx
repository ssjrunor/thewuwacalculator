...
import { mkSrcSttScp, sourceOptions } from '@/engine/services/sourceStateService.ts'
...
import { ResistanceGrid } from '@/modules/simulation/features/enemies/ResistanceGrid.tsx'
import {
  getEnemyState,
  getEnemyTune,
  getRslvEnemy,
...
import { listEffectsFor, listFfctForO, listStatesFor } from '@/data/catalog/gameDataService.ts'
  ...
  if (state.kind === 'toggle') {
    ...
    return (
      <div className={['res-combat-control', compact ? 'res-combat-control--compact' : '', checked ? 'is-active' : '', !enabled ? 'is-disabled' : ''].filter(Boolean).join(' ')}>
        <button
          type="button" className="res-combat-switch"
...
function ResCombatTuneCard({ card, runtime, enemy, simulation, onRtPdt }: ResCombatStatePrps) {
  ...
  return (
    <article className={active ? 'combat-tune-card is-active' : 'combat-tune-card'}>
      ...
      <RichDscr
        description={card.body} className="combat-tune-card__desc"
...
function ResCombatPlainCard({ card, runtime, enemy, simulation, onRtPdt }: ResCombatStatePrps) {
  ...
  return (
    <article className="pane-section">
      ...
      <div className="enemy-banner-card">
        <div className="enemy-banner">
          <div className="enemy-banner__top">
            <button
              type="button" className="resonator-avatar-button enemy-banner__avatar"
        ...
        <section className="enemy-strip enemy-res">
          <ResistanceGrid
            ...
            elements={ENEMYELEMPTN}
            editable={isCustomMode}
            shredFor={resShredFor}
            onChange={onNmyPrflChn}
          />
        </section>
      </div>
        ...
        <section className="enemy-strip">
          ...
          <div className="stack combat-effect-controls">
            {enemyPassives.map((effect) => (
              <div
                key={effect.id} className="pane-section"
              >
                <div className="sequence-card-head">
                  <div className="sequence-card-title-row">
                    <h4>{effect.label}</h4>
            ...
            {enemySourceCombatStates.map((state) => (
              <div
                key={state.controlKey} className="pane-section"
              >
                <EnemyCombatCtrl
                  state={state}
