...
export function echoStatTint(key: string): string {
  if (ATTRIBUTE_KEYS.has(key)) {
    return ATTR_COLORS[key as AttributeKey] ?? 'var(--picker-modal-accent, var(--resonator-accent, var(--accent)))'
  }
  return 'color-mix(in srgb, var(--picker-modal-muted, var(--muted)) 55%, var(--picker-modal-text, var(--text)))'
}
...
export function EchoStatGlyph({ statKey, size = 0.85 }: { statKey: string; size?: number }) {
  ...
  if (!icon) return null
  return (
    <span className="echo-stat-glyph"
      style={{
