...
export interface InventoryPaste {
  echoesToAdd: EchoInstance[]
  addedCount: number
  skippedCount: number
}

let cachedEchoClip: EchoClipPayload | null = null
...
export function cloneClipEchoes(echoes: EchoInstance[]): EchoInstance[] {
  return echoes.map((echo) => cloneEcho(echo))
}

export function makeEchoClip(args: {
  source: EchoClipSource
  resonatorId: ResonatorId
  resName: string
  echoes: EchoInstance[]
  ...
  return {
    ...
    echoes: cloneClipEchoes(args.echoes),
  }
}

export function serializeEchoClip(payload: EchoClipPayload): string {
  return encShareText({
    ...payload,
    echoes: cloneClipEchoes(payload.echoes),
  })
}

export function parseEchoClip(raw: string): EchoClipPayload | null {
  if (!raw) {
    return null
  }
...
export async function writeEchoClip(payload: EchoClipPayload): Promise<boolean> {
...
export async function readEchoClip(): Promise<EchoClipPayload | null> {
  if (typeof navigator === 'undefined' || !navigator.clipboard?.readText) {
    return cachedEchoClip
      ? { ...cachedEchoClip, echoes: cloneClipEchoes(cachedEchoClip.echoes) }
      : null
  }

  try {
    const text = await navigator.clipboard.readText()
    const parsed = parseEchoClip(text)
    ...
    cachedEchoClip = parsed
      ? { ...parsed, echoes: cloneClipEchoes(parsed.echoes) }
      : null
    return parsed
  } catch {
    return cachedEchoClip
...
export function pasteEchoes(
  ...
  for (const echo of payload.echoes) {
    ...
    if (nextTtlCost - curSlotCost + nextEchoCost > MAX_ECHO_COST) {
      skippedCount += 1
      slotIndex += 1
      continue
    }
...
export function resolveInventoryPaste(
  existingEchoes: EchoInstance[],
  payload: EchoClipPayload,
): InventoryPaste {
  ...
  for (const echo of payload.echoes) {
    const dplcXsts = existingEchoes.some((existing) => equalEchoes(existing, echo))
      || echoesToAdd.some((existing) => equalEchoes(existing, echo))
