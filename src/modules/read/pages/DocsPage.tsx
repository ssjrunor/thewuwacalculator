...
function scorePct(score: number, baseline: number, reference: number, maximum: number): number {
...
function AnchorScale() {
  ...
  const ticks = [
    ...
    { key: 'reference', label: 'Reference', pct: '100', frac: fracOf(reference) },
    { key: 'maximum', label: 'Maximum', pct: '200', frac: fracOf(maximum) },
  ]
      ...
      <div className="docs-scale__readout" aria-hidden="true">
        ...
        <div className="docs-scale__readout-meta">
          ...
          <span className="docs-scale__dmg">
            <span className="docs-scale__flat">{Math.round(flatPct)}% of maximum dmg</span>, {Math.round(damage)}k dmg</span>
        </div>
      </div>

      <div className="docs-scale__instrument">
        <div
          ref={trackRef} className="docs-scale__track"
          role="slider"
          tabIndex={0}
          aria-label="Build damage, illustrative"
          aria-valuemin={0}
          aria-valuemax={Math.round(scorePct(domainMax, baseline, reference, maximum))}
        ...
        <div className="docs-scale__legend" aria-hidden="true">
          {ticks.map((tick) => (
            <button
              key={tick.key}
              type="button" className="docs-scale__legend-item"
...
function StackRamp() {
  ...
  return (
    <div className="docs-ramp" style={{ '--fx': color } as CssProps}>
      <div className="docs-ramp__chips" role="tablist" aria-label="Negative effect">
        {RAMP_EFFECTS.map((fx, idx) => (
          <button
            ...
            type="button"
            role="tab"
            aria-selected={idx === effectIdx} className="docs-ramp__chip"
      ...
      <div
        ref={trackRef} className="docs-ramp__track"
        role="slider"
        tabIndex={0}
      ...
      >
        {bars.map((base, i) => {
          ...
          return (
            <span
              key={stack} className="docs-ramp__bar"
    ...
    <div className="docs-space__lever" data-disabled={disabled || undefined}>
      ...
      <div className="docs-space__seg" role="group" aria-label={label}>
        {options.map((opt) => (
          <button
            key={opt.id}
            type="button" className="docs-space__seg-btn"
...
function SearchSpace() {
  ...
  return (
    <div className="docs-space">
      ...
      <div className="docs-space__meter"
        role="img"
        ...
        {ticks.map((p) => (
          <span
            key={p} className="docs-space__grid"
      ...
      <div className="docs-space__axis" aria-hidden="true">
        {ticks.map((p) => (
          <span
            key={p} className="docs-space__tick"
            style={{ left: `${((p - decMin) / (decMax - decMin)) * 100}%` }}
          >
            10<sup>{p}</sup>
          </span>
    ...
    <figure className="docs-figure">
      ...
      <div className="docs-leveltable__scroll" data-expanded={expanded || undefined}>
        <div className="docs-table" role="table" style={{ '--cols': 2 } as CssProps}>
          <div className="docs-table__row" role="row">
      ...
      <button
        type="button" className="docs-leveltable__toggle"
                ...
                {docTopics.map((t, i) => {
                  ...
                  return (
                    <button
                      ...
                      role="tab"
                      aria-selected={isActive}
                      tabIndex={isActive ? 0 : -1} className="docs-mcard"
                      data-active={isActive || undefined}
              ...
              <nav className="docs-snav" aria-label="Sections in this method">
                {topic.sections.map((section) => {
                  ...
                  return (
                    <button
                      key={section.id}
                      type="button" className="docs-snav__item"
