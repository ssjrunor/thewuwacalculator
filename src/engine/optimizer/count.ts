/*
  Author: Runor Ewhro
  Description: Counts optimizer search space size under different counting
               modes, with support for locked main echoes and cost-limited
               5-echo combinations.
*/
...
export function countOptimizerCombinationsByMode(
  ...
  if (!lockedMainEchoId) {
    ...
    let combos = 0
    const row = dp[OPTIMIZER_ECHOS_PER_COMBO]

    // sum all exact 5-echo combinations whose total cost is within the cap
    for (let cost = 0; cost <= OPTIMIZER_MAX_COST; cost += 1) {
      combos += row[cost]
    }
