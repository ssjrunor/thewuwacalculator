...
// current optimizer search assumes all special stateful set variants are available,
// so runtime controls do not prune which set rows can be considered.
export function buildSetRuntimeMask(_runtime: ResonatorRuntimeState): number {
  void _runtime
  return SET_RUNTIME_TOGGLE_ALL
}
...
export function buildSetRows(_runtime: ResonatorRuntimeState): Float32Array {
  void _runtime
