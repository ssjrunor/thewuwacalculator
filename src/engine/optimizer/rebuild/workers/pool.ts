...
} from '@/engine/optimizer/rebuild/target/execution'
import {
  createPackedRotationExecution,
  sharePackedRotationExecution,
} from '@/engine/optimizer/rebuild/rotation/execution'
import type {
  OptimizerBackend,
...
import {
  ...
  type TargetJobSpec,
} from '@/engine/optimizer/rebuild/workers/targetGpu'
...
function createWorkerHandle(): OptimizerPoolWorker {
  const worker = new Worker(
      new URL('@/engine/optimizer/rebuild/task.worker.ts', import.meta.url),
      { type: 'module' },
  )
