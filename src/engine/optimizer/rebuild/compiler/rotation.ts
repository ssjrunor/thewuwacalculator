...
import { selectOptimizerTargetSkill } from '@/engine/optimizer/rebuild/target/selectedSkill'
import type { CompiledTargetSkillContext, OptimizerStartPayload, PreparedRotationRun } from '@/engine/optimizer/types'
...
import { buildGenericMainEchoRows, encodeEchoRows } from '@/engine/optimizer/rebuild/encode/echoes'
...
import { isOptimizerRotationTarget } from '@/engine/optimizer/rebuild/eligibility'
import {
...
export function compileRotationRun(input: OptimizerStartPayload): PreparedRotationRun {
  ...
  const setConstLut = buildSetRows(rotationRuntime)
