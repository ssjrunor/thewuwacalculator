/*
  Author: Runor Ewhro
  Description: Discovers evaluation echo, main-stat, Sonata, and main-Echo candidates.
*/
...
import { getEchoById, listChsByCos, listEchoes } from '@/data/catalog/echoCatalogService';
...
import { isMaxSetPlan, isUtilitySet, makeEffectiveSetPlan } from '@/engine/gameData/sonataPlan';
import type { EvaluationBuildSnapshot, EvaluationEchoSlot, EvaluationFeature, EvaluationFeatureGroup, EvaluationFeatureGroups, EvaluationOverviewStats, EvaluationSetSummary, EvaluationStatContribution, EvaluationSubstatEntry, EvaluationSubstatMode } from './types.ts';
import { addStatTotal, collectMainStatSources, formatFeatureTabLabel, getEvaluationStatKeys, type EvaluationEchoFrame, type MainEchoChoice, type MainEchoProfile, type MainStatCandidate, type MainStatSourceSummary } from './stats.ts';
...
export function makeSetSummary(setRows: Uint8Array, echoes: EchoInstance[]): EvaluationSetSummary[] {
...
export function makeReferenceEvaluationEchoes(
  ...
  for (let index = 0; index < orderedCostPlan.length; index += 1) {
    ...
    echoes.push({
      uid: `evaluation-reference-${orderedCostPlan.join('')}-${index + 1}`,
      id: definition.id,
      set,
      mainEcho: false,
      mainStats: {
...
export function makeMainEchoProfiles(ctx: SuggestContext): MainEchoProfile[] {
  ...
  return fixtures.map(({ def, echo }, index) => {
    ...
    const buffed = makeEvaluationEchoFrame(ctx, [echo], buffs)
    const neutral = makeEvaluationEchoFrame(ctx, [echo], new Float32Array(MAIN_BUFF_LEN))
...
export function makeEvaluationBuildSnapshot({
  label,
  score,
  damage,
  echoes,
...
export function makeFeatureGroups(features: EvaluationFeature[]): EvaluationFeatureGroups {
  const bySkillType = new Map<string, EvaluationFeatureGroup & { weightedDamage: number }>()
  const byTab = new Map<string, EvaluationFeatureGroup & { weightedDamage: number }>()

  const add = (
    map: Map<string, EvaluationFeatureGroup & { weightedDamage: number }>,
    key: string,
    label: string,
  ...
  const normalize = (map: Map<string, EvaluationFeatureGroup & { weightedDamage: number }>) => {
...
export function makeStatContributions(
  ...
  const rows = getEvaluationStatKeys(mains, substats).map((key) => {
...
let evaluationProbeValues: Record<string, number> | null = null

export function getEvaluationProbeValues(): Record<string, number> {
  if (evaluationProbeValues) return evaluationProbeValues
  ...
  evaluationProbeValues = probeValues
  return probeValues
}
...
export function findUsefulStatImpacts(
  ...
  const probeValues = getEvaluationProbeValues()
...
export function makeEvaluationEchoFrame(
  ctx: SuggestContext,
  echoes: EchoInstance[],
