/*
  Author: Runor Ewhro
  Description: opt-in long stress test for evaluation anchor stability across
...
import { makeEvaluationCostPlans } from '@/engine/evaluation/evaluation/costPlans'
...
import { SET_RULES, evaluationSetConditions } from '@/engine/evaluation/setStatePolicy'
import {
  assembleEvaluation,
  evaluationErTarget,
...
import {
  ...
  utilityPlanFor,
} from '@/engine/evaluation/evaluation/echoDiscovery.ts'
import type { EvaluationBuildSnapshot } from '@/engine/evaluation/evaluation/types.ts'
import { isUtilitySet } from '@/engine/gameData/sonataPlan'
import { applyEvaluationAsm, EVALUATION_ENEMY } from '@/modules/simulation/model/evaluationAssumptions'
...
const RUN_STRESS = TEST_ENV.WUWA_EVALUATION_STRESS === '1'
...
function legalSetIds(resonatorId: string): number[] {
  // stress inputs avoid utility sets and resonators without evaluation set rules
...
function maxedRuntime(seed: ResSeed, sequence: number): ResRuntime {
  ...
  return applyEvaluationAsm(maxed)
}
...
function randomEchoes(resonatorId: string, scenarioIndex: number): Array<EchoInstance | null> {
  ...
  const costPlan = pick(rng, makeEvaluationCostPlans())
...
function buildContext(seed: ResSeed, runtime: ResRuntime): SuggestContext {
  // stress scoring uses rotation-mode context with evaluation assumptions applied
  // so exhaustive anchors match the app's default evaluation report path
  ...
  const context = mkSuggVltnCt({
    scenarioId: combatScenarioId('evaluation:stress'),
    ...
    runtimesById,
    selectedTargets: {},
    setConds: evaluationSetConditions(runtime.id),
  ...
  if (!context) {
    throw new Error(`No evaluation context for ${runtime.id}`)
  }

  return context
}

function buildSignature(build: EvaluationBuildSnapshot) {
  return {
...
function anchorInputSignature(ctx: SuggestContext, equipped: Array<EchoInstance | null>) {
  ...
  return {
    targetEr: round(evaluationErTarget(ctx, equipped), 4),
...
function expectStableAnchors(resonatorId: string): void {
  ...
  let referenceAnchors: EvaluationAnchors | null = null
  ...
  for (let scenario = 0; scenario < SCENARIOS_PER_RESONATOR; scenario += 1) {
    ...
    if (!referenceAnchors || scenario === SCENARIOS_PER_RESONATOR - 1) {
      const anchors = buildEvaluationAnchors(context, equipped)
      if (!anchors) {
        throw new Error(`No evaluation anchors for ${resonatorId} scenario ${scenario}`)
      }
    ...
    const score = assembleEvaluation(context, equipped, referenceAnchors)
    ...
    expect(score.referenceDamage).toBe(referenceAnchors.referenceDamage)
    expect(score.maximumDamage).toBe(referenceAnchors.maximumDamage)
    checked += 1
  }
...
describe.runIf(RUN_STRESS)('evaluation anchor stress', () => {
...
describe.skipIf(RUN_STRESS)('evaluation anchor stress', () => {
  it('is opt-in because it performs 100 exhaustive evaluation searches', () => {
    expect(TEST_ENV.WUWA_EVALUATION_STRESS).not.toBe('1')
  })
})
